...
CREATE TABLE `ORDEN` (
  ...
  `FK_ID_CLIENTES` VARCHAR(10) NOT NULL,
  `FK_PATENTE` VARCHAR(8) NOT NULL,
  `FK_ESTADO` INT NOT NULL,
  `FK_ID_REPORTE` int UNIQUE
);
...
ALTER TABLE `REPORTE` ADD CONSTRAINT `ORDEN_REPORTE` FOREIGN KEY (`ID_REPORTE`) REFERENCES `ORDEN` (`FK_ID_REPORTE`);

ALTER TABLE `DOCUMENTO` ADD CONSTRAINT `DOCUMENTOS_CLIENTES` FOREIGN KEY (`FK_ID_CLIENTE`) REFERENCES `CLIENTE` (`ID_CLIENTE`);

ALTER TABLE `ORDEN` ADD CONSTRAINT `ORDENES_CLIENTES` FOREIGN KEY (`FK_ID_CLIENTES`) REFERENCES `CLIENTE` (`ID_CLIENTE`);
...
ALTER TABLE `EMPLEADO` ADD CONSTRAINT `USUARIO_EMPLEADO` FOREIGN KEY (`FK_ID_USUARIO`) REFERENCES `USUARIO` (`ID_USUARIO`);
